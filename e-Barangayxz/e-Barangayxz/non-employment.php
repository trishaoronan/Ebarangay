...
<head>
  ...
  <title>Barangay Certificate of Non-Employment</title>
...
<body class="doc-page">
  ...
  <section class="page-header mb-4">
    <div class="container">
      <h2 class="fw-bold mb-2 text-dark">Certificate of Non-Employment Request</h2>
      <p class="text-muted">Request a certification proving that the applicant is not gainfully employed within the Barangay.</p>
    </div>
  </section>
  ...
  <div class="container pb-5">
    <div class="form-card">
      <form class="row g-3" id="nonEmploymentForm" action="#" method="post" enctype="multipart/form-data" novalidate>
        ...
        <div class="col-12">
          ...
          <select class="form-select" id="specificPurpose" name="specificPurpose" required>
            ...
            <option value="SSS Claim">SSS Claim</option>
        ...
        <div class="col-12 mt-2">
          <div class="form-check border rounded p-3 bg-light">
            <input type="checkbox" class="form-check-input me-2 mt-1" name="declaration" value="yes" required />
            <p class="mb-0">I hereby attest and declare under oath that I am <strong>CURRENTLY UNEMPLOYED</strong> and have no stable source of income or gainful employment at the time of this application.</p>
          </div>
        </div>
        ...
        <div class="col-md-6">
          ...
          <select class="form-select" id="modeOfRelease" name="modeOfRelease" required>
            <option value="">Select</option>
        ...
        <div class="col-12 text-end mt-3">
          ...
          <button type="button" id="submitBtn" class="btn btn-danger px-4">Submit</button>
          <div id="submitDebug" class="small text-muted mt-2"></div>
        </div>
  ...
  <script>
    ...
    if (window.IdValidation) window.IdValidation.initialize('nonEmploymentForm');
    ...
    document.addEventListener('DOMContentLoaded', function() {
      initializeNameValidation('nonEmploymentForm');

      const form = document.getElementById('nonEmploymentForm');
      ...
      submitBtn.addEventListener('click', async function () {
        const dbg = document.getElementById('submitDebug');
        if (dbg) dbg.textContent = 'Processing...';
        console.log('v3: Submit button clicked');
        ...
        const modeOfReleaseEl = document.getElementById('modeOfRelease');
        const specificPurposeEl = document.getElementById('specificPurpose');
        const civilStatus = civilStatusEl ? civilStatusEl.value : 'Single';
        let specificPurpose = specificPurposeEl ? specificPurposeEl.value.trim() : 'Not specified';
        const modeOfRelease = modeOfReleaseEl ? modeOfReleaseEl.value : 'Pickup';

        // If "Others" is selected, use the custom text
        if (specificPurpose === 'Others') {
          const otherText = document.getElementById('otherPurposeText');
          if (otherText && otherText.value) {
            specificPurpose = otherText.value;
          }
        }

        const formData = new FormData(form);
        // Explicitly set values to ensure they are included correctly
        formData.set('civilStatus', civilStatus);
        formData.set('specificPurpose', specificPurpose);
        formData.set('modeOfRelease', modeOfRelease);
        if (dbg) {
          dbg.textContent = `Submitting: civilStatus="${civilStatus}", purpose="${specificPurpose}", mode="${modeOfRelease}"`;
        }
        if (window.console) {
          console.log('ne-frontend-v1: preparing submission');
          for (const [key, value] of formData.entries()) {
            console.log('FormData', key, value);
          }
        }
        try {
          const response = await fetch('submit_non_employment.php?ts=' + Date.now(), { method: 'POST', body: formData, cache: 'no-store', credentials: 'include', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
          const data = await response.json();

          if (data.success) {
            const defaultMsg = 'Your Certificate of Non-Employment request has been submitted successfully. Verification will be conducted, and you will be notified when the certificate is ready.';
            if (successMessageEl) {
              successMessageEl.textContent = data.message || defaultMsg;
            }
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content border-0 shadow-lg rounded-4">
        ...
        <div class="modal-body text-center py-5">
          ...
          <p class="text-muted mb-0" id="successMessage">Your Certificate of Non-Employment request has been submitted successfully. Verification will be conducted, and you will be notified when the certificate is ready.</p>
        </div>
