...
try {
    ...
    $required = ['lastName','firstName','dateOfBirth','completeAddress','contactNumber','childrenCount','reason','modeOfRelease','modeOfPayment'];
    ...
    // Upload directories
    $idDir = 'uploads/solo_parent_ids/';
    $suppDir = 'uploads/solo_parent_supporting/';
    if (!file_exists($idDir)) mkdir($idDir, 0777, true);
    if (!file_exists($suppDir)) mkdir($suppDir, 0777, true);

    // Save valid ID
    $idExt = pathinfo($validId['name'], PATHINFO_EXTENSION);
    $validIdPath = $idDir . 'spid_' . $resident_id . '_' . time() . '.' . $idExt;
    if (!move_uploaded_file($validId['tmp_name'], $validIdPath)) {
        throw new Exception('Failed to upload valid ID');
    }

    // Save supporting documents (optional, multiple)
    $supportingPaths = [];
    if (isset($_FILES['supportingDocs'])) {
        foreach ($_FILES['supportingDocs']['name'] as $idx => $name) {
            if ($_FILES['supportingDocs']['error'][$idx] === UPLOAD_ERR_OK && $_FILES['supportingDocs']['name'][$idx] !== '') {
                $tmp = $_FILES['supportingDocs']['tmp_name'][$idx];
                $type = $_FILES['supportingDocs']['type'][$idx];
                $size = $_FILES['supportingDocs']['size'][$idx];
                if (!in_array($type, $allowedTypes)) continue; // skip invalid types silently
                if ($size > $maxFileSize) continue; // skip oversize
                $ext = pathinfo($name, PATHINFO_EXTENSION);
                $dest = $suppDir . 'supp_' . $resident_id . '_' . time() . '_' . $idx . '.' . $ext;
    ...
    // Ensure solo_parent table exists
    ...
    if (!$conn->query($tableCreate)) {
        throw new Exception('Database error creating solo_parent table');
    }

    // Insert into requests table
    $documentType = 'Solo Parent Certificate';
    $status = 'pending';
    ...
    addResidentActivity(
        ...
        "Your request for Solo Parent Certificate has been submitted and is pending review.",
        null,
    ...
    $childrenAges = isset($_POST['childrenAges']) ? trim($_POST['childrenAges']) : null;
    $reason = trim($_POST['reason']);
    $modeOfRelease = $_POST['modeOfRelease'];
    $supportingText = count($supportingPaths) ? json_encode($supportingPaths) : null;

    // Insert into solo_parent
    $stmt = $conn->prepare("INSERT INTO solo_parent (request_id, resident_id, last_name, first_name, middle_name, suffix, date_of_birth, civil_status, complete_address, contact_number, children_count, children_ages, reason, valid_id_path, supporting_paths, mode_of_release) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) throw new Exception('Database error preparing solo_parent insert');
    ...
    addStaffNotification(
        ...
        "$resident_name has requested Solo Parent Certificate",
        $request_id,
        'request'
    );
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Solo Parent Certificate request submitted successfully!',
    ...
    if (!empty($supportingPaths)) {
        foreach ($supportingPaths as $p) { if (file_exists($p)) unlink($p); }
    }
